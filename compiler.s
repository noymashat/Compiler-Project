...
;;; Creates a literal string with value %1 
...
;;; Creates a literal with tag %1 
;;; and the definition %2 of the value %3 
%macro MAKE_LITERAL 3 									
	db %1 
	%2 %3
%endmacro

%macro APPLY_MAKE_REV_LIST 1 	; %1 = a pointer to last param of apply (list). keep rax!
	push rcx
	push rdx
	push r13
	mov rcx, %1
...
%macro APPLY_PUSH_LIST_ARG 2 	; %1 = a pointer to the reversed list, %2 = args counter - keep r9!
	mov rcx, %1
%%push_list_loop:
	mov dl, byte [rcx]
    cmp dl, T_NIL
	je %%end_push_list_loop
	CAR rdx, rcx
    CDR rcx, rcx
	push rdx
	inc %2
...
%macro APPLY_PUSH_REST_ARGS 2 	; PVAR(%1) = last arg (before list arg), %2 = args counter - keep r9!
	mov rcx, %1
%%push_rest_loop:
	cmp rcx, qword 0
	je %%end_push_rest_loop
	mov r13, PVAR(rcx)
	push r13
	inc %2
	dec rcx
...
%macro SHIFT_FRAME 1    ; %1 = size of SHIFT_FRAME
    push rax
	push rcx
	push rdx
	push r13	
...
%%end_shift_loop:
	...
	pop rax
	shl r15, 3 
	add rsp, r15		; correct rsp
%endmacro

%macro MAKE_OPT_LIST 2 	; %1 = opt_len, %2 = mand_len
	push r13
	push r14
	push rcx
	push rdx
    mov rcx, %1			; num of iter
    
	mov rdx, %1			; rdx = magic
	add rdx, %2			; rdx = magic

    mov r13, qword [rsp+(7+rdx)*WORD_SIZE]	; opt_0
    
%%opt_loop:
	cmp rcx, qword 0
	je %%end_opt_loop
    dec rdx
    mov r14, qword [rsp+(7+rdx)*WORD_SIZE] ; next_opt from end to start
    MAKE_PAIR (rax, r14, r13)
	mov r13, rax
	dec rcx
    jmp %%opt_loop
...
%%end_opt_loop:
	...
	dec rcx
	add rcx, %2
	mov qword [rsp+(7+rcx)*WORD_SIZE], r13	; save opt_list as last arg
	pop rdx
	pop rcx
	pop r14
	pop r13
...
%macro SHRINK_FRAME 3    ; %1 = mand_len+3, %2 = opt_len, %3 = arg_count-1
	push rcx	
	push r14
	push r13
	mov rcx, qword 1
%%shrink_loop:			; shift stack frame from end to start (from last mand only)
